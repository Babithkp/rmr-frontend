...
export default function OrderUpdate() {
  ...
  const [ordeItems, setOrdereItems] = useState<OrderItemUpdate[]>([]);
    ...
    if (orderId) {
      ...
      if (response?.status === 200) {
        ...
        const items: OrderItemUpdate[] = orders.Items.map((orderItem) => {
          ...
          const unitPriceWithGst = basePrice + (basePrice * gst) / 100;
          const totalPrice = orderItem.quantity * unitPriceWithGst;
  ...
  return (
    <main className="flex w-full flex-col gap-5 px-20 max-xl:px-5 max-xl:text-[10px] max-sm:px-2 ">
      ...
      <section className="flex gap-5 max-sm:flex-col max-sm:gap-5">
          ...
          <table className="w-full">
            ...
            <tbody>
              {ordeItems.map((item, i) => (
                <tr key={i} className="border border-t">
        ...
        <div className="flex h-[85vh] w-[30%] flex-col justify-between overflow-y-auto rounded-lg border p-2 max-sm:w-full">
